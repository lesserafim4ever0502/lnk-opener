...
class MarkdownPreviewPanel {
    ...
    _isUserScrollingEditor = false;
    _userScrollEditorTimeout;
    static createOrShow(extensionUri) {
        ...
        const panel = vscode.window.createWebviewPanel('mdLnkPreview', 'Markdown 预览', viewColumn || vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    constructor(panel, extensionUri, document) {
        ...
        vscode.window.onDidChangeTextEditorVisibleRanges(e => {
            if (e.textEditor.document === this._document) {
                if (this._isProgrammaticReveal)
                    return;
                this._isUserScrollingEditor = true;
                if (this._userScrollEditorTimeout)
                    clearTimeout(this._userScrollEditorTimeout);
        ...
        this._panel.webview.onDidReceiveMessage(message => {
            switch (message.command) {
                case 'openLnk':
                    try {
                        ...
                        if (fs.existsSync(absolutePath)) {
                            (0, child_process_1.execFile)('explorer.exe', [absolutePath]);
                        }
                        else {
                ...
                case 'openLocal':
                    try {
                        ...
                        if (fs.existsSync(localPath)) {
                            (0, child_process_1.execFile)('explorer.exe', [localPath]);
                        }
                    }
                    catch (e) { }
                    return;
                case 'openExternal':
                    ...
                    return;
                case 'revealLine':
                    if (this._isUserScrollingEditor)
                        return;
                    if (this._document) {
    ...
    async bulkResolveLnkAsync(lnkPaths) {
        ...
        return new Promise((resolve) => {
            ...
            (0, child_process_1.execFile)('powershell.exe', ['-NoProfile', '-Command', script], { encoding: 'utf8' }, (err, stdout) => {
                if (stdout) {
